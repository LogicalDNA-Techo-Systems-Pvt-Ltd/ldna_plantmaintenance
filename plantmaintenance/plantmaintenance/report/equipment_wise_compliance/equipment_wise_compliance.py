...
def get_data(filters):
    ...
    query = f"""
        SELECT
            ...
            COUNT(DISTINCT CASE WHEN t.status = 'Completed' THEN t.name END) AS completed_count,
        ...
        LEFT JOIN
            `tabTask Detail` AS t ON t.equipment_code = e.equipment_code
        LEFT JOIN (
            SELECT 
                equipment_code,
    ...
    return [
        {
            ...
            'compliance_ratio': round((row['completed_count'] / row['task_count']), 2) if row['task_count'] else 0,
