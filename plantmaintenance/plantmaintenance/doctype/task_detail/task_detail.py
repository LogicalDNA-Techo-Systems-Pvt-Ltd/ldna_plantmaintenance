...
def generate_future_tasks():
    ...
    for equipment_code in equipment_list:
        all_tasks = frappe.db.get_all(
            ...
            fields=["activity", "parameter", "frequency", "plan_start_date", "activity_group"]
        )

        task_groups = {}
        for task in all_tasks:
            key = (task.activity, task.parameter, task.frequency)
            if key not in task_groups or getdate(task.plan_start_date) > getdate(task_groups[key]["plan_start_date"]):
        ...
        for key, task in task_groups.items():
            activity, parameter, frequency = key
            if not parameter:
                continue

            parameter_doc = frappe.get_doc("Parameter", parameter)
    ...
    if tasks_generated:
        frappe.logger().info(f" {tasks_generated} tasks generated up to 31/12/2028.")
    else:
        frappe.logger().info(" All tasks already generated up to 31/12/2028. No new tasks created.")
