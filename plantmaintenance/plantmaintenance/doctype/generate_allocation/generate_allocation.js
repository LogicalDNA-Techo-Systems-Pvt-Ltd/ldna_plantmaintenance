...
frappe.ui.form.on('Generate Allocation', {
    ...
    work_center: function (frm) {
        if (frm.doc.work_center) {
            frm.set_value('equipment', '');
            
            frappe.call({
                method: 'plantmaintenance.plantmaintenance.doctype.equipment.equipment.get_equipment_based_on_work_center',
                args: {
                ...
                callback: function (response) {
                    if (response.message) {
                        console.log(response.message);
                        var equipmentList = response.message;
                        frm.set_query('equipment', () => {
                            return {
                                filters: {
                                    name: ['in', equipmentList]
                                }
                            };
                        });
                    }
...
function load_tasks(frm) {
    ...
    if (frm.doc.equipment && Array.isArray(frm.doc.equipment)) {
        equipment_list = frm.doc.equipment.map(e => e.equipment);  
    }

    frappe.call({
        method: 'plantmaintenance.plantmaintenance.doctype.generate_allocation.generate_allocation.load_tasks',
        args: {
            plant: frm.doc.plant || null,
            location: frm.doc.location || null,
            plant_section: frm.doc.plant_section || null,
            work_center: frm.doc.work_center || null,
            start_date: frm.doc.start_date,
            end_date: frm.doc.end_date,
            equipment_list: equipment_list.length ? JSON.stringify(equipment_list) : null 
        },
        callback: function (response) {
            if (response.message && Array.isArray(response.message)) {
                var tasks = response.message.sort((a, b) => new Date(a.date) - new Date(b.date));
                ...
                let downloadBtn = document.getElementById('download-tasks-excel-btn');
                let uploadBtn = document.getElementById('upload-assignment-excel-btn');
...
function download_tasks_excel(tasks) {
    frappe.call({
        method: 'plantmaintenance.plantmaintenance.doctype.generate_allocation.generate_allocation.download_tasks_excel_for_allocation',
        args: {
...
function upload_assignment_excel(frm) {
    frappe.prompt([
        ...
        frappe.call({
            method: 'plantmaintenance.plantmaintenance.doctype.generate_allocation.generate_allocation.upload_tasks_excel_for_allocation',
            args: {
